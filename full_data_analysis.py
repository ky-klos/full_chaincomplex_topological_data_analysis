...
def full_analysis(device,measures,simulation_data_path,data_type, sim_type,defect_nmb,temperatures,output_data_path,new_data=False,new_basis_data=False,less_idx_list = [False,True],configuration_attributes_dict=None,gen_folder='./',save_basis_data=False,larger=False,compare_defect_lattice=False,full_sample_size =1000,tda_sample_size=100,noise_size = 1.0,epochs=[-1],lattice_size=16,training_data_nmb=350000,nmb_label=10):
    from filtration_function import TopologicalAnalysis
    from observables_analysis_class_26 import Observables
    import os
    if data_type == "generated_data":
    ...
    for t_idx,t in enumerate(temperatures):
            if 'statistical_analysis' in measures:
                from observables_calculation_26 import calculation_observables
